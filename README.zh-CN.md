...
**一个让桌面 AI 拥有独立实体语音终端的 ESP32-S3 硬件项目。**

已完成的原始系统此前实现过如下真实硬件端到端产品闭环：

**麦克风 → ESP32-S3 → USB → 桌面端 → ASR → AI/Agent → TTS → USB → ESP32-S3 → 扬声器**

核心工程难点在于：持续音频流与控制消息共享一条 USB 字节流时，仍须保证传输可靠——有界 framing、从分片或合并读取中恢复、明确设备生命周期行为，以及无需物理设备的主机端可测试性。

本仓库是从该已完成私有系统中提取出的公开工程版本：它作为参考实现，展示可复用的二进制 framing、USB 串口传输、协议边界和 Node.js 主机端可靠性核心。完整的桌面 AI 应用、ASR/TTS 集成和生产配置均有意不公开。

当前自动化验证为 **OFFLINE**。上文的真实硬件闭环仅是私有系统的历史证据，并不代表此公开仓库当前的硬件验收。
